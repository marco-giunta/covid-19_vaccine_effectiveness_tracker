world.map <- function() {
    world_data = read_csv('owid-covid-data.csv', col_types = cols())
    #world_data
    ...
    ggplot(data = world_map) + 
        ...
        coord_fixed(1.3) + ggtitle('Percentage of vaccinated people per nation (at least one dose), July 2021') +
...
barcomparator <- function(lista){
    ...
    up <- c()
    down <- c()
    title <- c('Pfizer old', 'Pfizer new', 'Moderna', 'Vaxzevria', 'Janssen')
...
pfizer.age.fun <- function(df = pfizer.df, pv = pfizer.pv, pp = pfizer.pp, col = c("#FFCC00","#FFCC66","#CC9900","#996600")) {
    pfizer.age <- ageinference(df, pv, pp)
...
moderna.age.fun <- function(df = moderna.df, pv = moderna.pv, pp = moderna.pp, col = c("#FF3300","#CC3300","#990000")) {
    moderna.age <- ageinference(df, pv, pp)
...
janssen.age.fun <- function(df = janssen.df, pv = janssen.pv, pp = janssen.pp, col = c("#99FF33","#339900")) {
    janssen.age <- ageinference(df, pv, pp)
